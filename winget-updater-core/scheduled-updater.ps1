...
if (Get-Command wt.exe -ErrorAction SilentlyContinue) {
	Start-Process "wt.exe" -ArgumentList "-w new powershell.exe -NoProfile -ExecutionPolicy Bypass -File `"$PSScriptRoot\winget-updater.ps1`" -Minimal -CachePath `"$tempCache`"" -WindowStyle Normal
}
else {
	Start-Process "powershell.exe" -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$PSScriptRoot\winget-updater.ps1`" -Minimal -CachePath `"$tempCache`"" -WindowStyle Normal
}